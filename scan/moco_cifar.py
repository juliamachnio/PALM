...
from utils.common_config import get_val_transformations_resize
...
def main():
    ...
    transforms = get_val_transformations_resize(p)
    ...
    #memory_bak_train in moco.py is the same as memory_bank_base in simclr.py
    topk = 50
    ...
    np.save(p['pretext_features'], memory_bank_train.pre_lasts.cpu().numpy())

    fill_memory_bank(val_dataloader, model, memory_bank_val)
    val_features = memory_bank_val.pre_lasts.cpu().numpy()
    print("Validation Features Shape:", val_features.shape)
    print("Validation Features Sample:", val_features)
