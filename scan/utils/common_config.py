...
def get_criterion(p):
    ...
    elif p['criterion'] == 'dinov2':
        from losses.losses import DINOLoss
        criterion = DINOLoss(**p['criterion_kwargs'])
    else:
...
def get_feature_dimensions_backbone(p):
    if p['backbone'] == 'resnet18':
        ...
        return 2048
    elif p['backbone'] == 'vit_small':
        return 384
    elif p['backbone'] == 'vit_base':
        return 768
    elif p['backbone'] == 'vit_large':
        return 1024
    else:
        raise NotImplementedError(f"Backbone {p['backbone']} not implemented")
...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        ...
        if p['train_db_name'] in ['cifar-10', 'cifar-100']:
            ...
            backbone = resnet18()
        elif p['train_db_name'] == ['tiny-imagenet', 'imagenet_50']:
            from models.resnet_tinyimagenet import resnet18
            backbone = resnet18()
        elif p['train_db_name'] == 'svhn':
            from models.resnet_cifar import resnet18
            backbone = resnet18()
        else:
        ...
        if 'imagenet' in p['train_db_name']:
            ...
            backbone = resnet50()
        elif 'cifar-10' in p['train_db_name']:
            from models.resnet import resnet50
            backbone = resnet50()

        else:
            raise NotImplementedError
    elif p['backbone'] in ['vit_small', 'vit_base', 'vit_large']:
        from models.dino_vit import DinoViT
        backbone = DinoViT(p['backbone_kwargs'])
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        is_contrastive = to_augmented_dataset  # Only use two views for contrastive training
        dataset = SVHN(root=os.path.join(MyPath.db_root_dir('svhn')), split='train', transform=transform, contrastive=is_contrastive)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] == 'svhn':
        from data.svhn import SVHN  # Don't forget to import at the top
        dataset = SVHN(root=os.path.join(MyPath.db_root_dir('svhn')), split='test', transform=transform)
    else:
...
def get_val_transformations_resize(p):
    return transforms.Compose([
