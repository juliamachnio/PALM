...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, teacher_temp=0.04, student_temp=0.1, center_momentum=0.9):
        """
        DINO Loss: Self-distillation without labels.

        Args:
            out_dim (int): Number of output dimensions.
            teacher_temp (float): Temperature for the teacher model.
            student_temp (float): Temperature for the student model.
            center_momentum (float): Momentum for the teacher's center update.
        """
    ...
    def forward(self, student_output, teacher_output):
        ...
        Args:
            student_output (torch.Tensor): Student model's logits (batch_size, out_dim).
            teacher_output (torch.Tensor): Teacher model's logits (batch_size, out_dim).
        ...
        # Update the center (moving average of teacher outputs)
        self.center = self.center_momentum * self.center + (1 - self.center_momentum) * teacher_output.mean(dim=0,
                                                                                                            keepdim=True)
