...
import time  # already standard lib, no need to install
...
def main(cfg):
    # Setting up GPU args
    # total_sum = 0
    # rankk_test_vall.append(total_sum)
    # total_sum_train = 0
    # rankk_train_vall.append(total_sum_train)
    ...
    if os.path.exists(exp_dir):
        ...
        # Find all subfolders matching "episode_<number>"
        episode_dirs = [d for d in os.listdir(exp_dir)
                        if os.path.isdir(os.path.join(exp_dir, d)) and re.match(r'episode_\d+', d)]

        if episode_dirs:
            # Extract episode numbers
            episode_nums = [int(re.search(r'\d+', ep).group()) for ep in episode_dirs]
            max_episode = max(episode_nums)
            folder_to_remove = os.path.join(exp_dir, f"episode_{max_episode}")

            print(f"Removing previous episode folder: {folder_to_remove}")
            shutil.rmtree(folder_to_remove)

            # Set last_episode to the one we just removed
            args.last_episode = max_episode-1
            print(f"Resuming from episode {args.last_episode}")
    else:
        os.makedirs(exp_dir)
        print(f"Created new experiment directory: {exp_dir}")

    if args.last_episode != 0:
        # Remove last value from all tracking lists if not starting from scratch
        if plot_episode_xvalues: plot_episode_xvalues.pop()
        if plot_episode_yvalues: plot_episode_yvalues.pop()
        if plot_epoch_xvalues: plot_epoch_xvalues.pop()
        if plot_epoch_yvalues: plot_epoch_yvalues.pop()
        if plot_it_x_values: plot_it_x_values.pop()
        if plot_it_y_values: plot_it_y_values.pop()
        print(f"Trimmed plot values for resuming from episode {args.last_episode}")
    ...
    if args.last_episode == 0:
        ...
        print("episode {}".format(args.last_episode))
        cfg.ACTIVE_LEARNING.LSET_PATH = os.path.join(cfg.EXP_DIR,f"episode_{args.last_episode}", "lSet.npy")
        print(cfg.ACTIVE_LEARNING.LSET_PATH)
        cfg.ACTIVE_LEARNING.USET_PATH = os.path.join(cfg.EXP_DIR, f"episode_{args.last_episode}","uSet.npy")
        print(cfg.ACTIVE_LEARNING.USET_PATH)
        cfg.ACTIVE_LEARNING.VALSET_PATH = os.path.join(cfg.EXP_DIR, "valSet.npy")
        print(cfg.ACTIVE_LEARNING.VALSET_PATH)
        # cfg.ACTIVE_LEARNING.LSET_PATH = os.path.join(cfg.EXP_DIR,  "lSet.npy")
        # print(cfg.ACTIVE_LEARNING.LSET_PATH)
        # cfg.ACTIVE_LEARNING.USET_PATH = os.path.join(cfg.EXP_DIR,  "uSet.npy")
        # print(cfg.ACTIVE_LEARNING.USET_PATH)
        # cfg.ACTIVE_LEARNING.VALSET_PATH = os.path.join(cfg.EXP_DIR, "valSet.npy")
        # print(cfg.ACTIVE_LEARNING.VALSET_PATH)

    # lSet, uSet = data_obj.loadPartitions(lSetPath=cfg.ACTIVE_LEARNING.LSET_PATH, \
    ...
    print("lSet", lSet, len(lSet))
    print("uSet", uSet, len(uSet))
    # print("valSet", valSet, len(valSet))

    if args.last_episode > 0:
        prev_episode_dir = os.path.join(cfg.EXP_DIR, f"episode_{args.last_episode}")
        checkpoint_files = [f for f in os.listdir(prev_episode_dir) if f.endswith(".pycls")]

        if checkpoint_files:
            latest_ckpt = os.path.join(prev_episode_dir, checkpoint_files[0])
            print(f"Resuming model from checkpoint: {latest_ckpt}")
            model = model_builder.build_model(cfg).cuda()
            model = cu.load_checkpoint(latest_ckpt, model)
            print("Checkpoint successfully loaded.")
        else:
            print(f"No checkpoint found in {prev_episode_dir}. Starting from scratch.")
    ...
    if len(lSet) == 0:
        ...
        # ✅ Fix: Create episode_0 dir and set EPISODE_DIR
        episode_dir = os.path.join(cfg.EXP_DIR, 'episode_0')
        if not os.path.exists(episode_dir):
            os.mkdir(episode_dir)
        cfg.EPISODE_DIR = episode_dir

        val_ind = valSet
        al_obj = ActiveLearning(data_obj, cfg, val_ind)
    ...
    # print("lSet_loader", len(lSet_loader))
    # print("lSet_data", len(train_data))
    ...
    for cur_episode in range(args.last_episode+1, cfg.ACTIVE_LEARNING.MAX_ITER+1):
...
def test_model(test_loader, checkpoint_file, cfg, cur_episode, model):
    ...
    episode_file_x = os.path.join(cfg.EXP_DIR, "plot_episode_xvalues.npy")
    episode_file_y = os.path.join(cfg.EXP_DIR, "plot_episode_yvalues.npy")

    # Load previous episode values if the files exist
    if os.path.exists(episode_file_x) and os.path.exists(episode_file_y):
        try:
            plot_episode_xvalues = np.load(episode_file_x).tolist()
            plot_episode_yvalues = np.load(episode_file_y).tolist()
        except Exception as e:
            print(f"Error loading previous episode files: {e}")
            plot_episode_xvalues = []
            plot_episode_yvalues = []
    else:
        plot_episode_xvalues = []
    ...
    print(plot_episode_xvalues)
    print(plot_episode_yvalues)
    # Save updated values
    np.save(episode_file_x, np.array(plot_episode_xvalues))
    np.save(episode_file_y, np.array(plot_episode_yvalues))
...
if __name__ == "__main__":
    import time  # make sure it's imported at the top
    ...
    print(f"\nTotal runtime: {duration_min:.2f} minutes\n")
