...
def main(cfg):
    ...
    data_obj = Data(cfg)
    train_data, train_size = data_obj.getDataset(save_dir=cfg.DATASET.ROOT_DIR, isTrain=False, isDownload=True)
    ...
    # print("testSet_path {}".format(testSet_path))
    ...
    # print(cfg.ACTIVE_LEARNING.TESTSET_PATH)
...
#-----------------------
    ...
    test_data, test_size = data_obj.getDataset(save_dir=cfg.DATASET.ROOT_DIR, isTrain=False, isDownload=True)
    testSet_path = data_obj.makeTestSet(data=test_data, seed_id=cfg.RNG_SEED, save_dir=cfg.EXP_DIR)
    cfg.ACTIVE_LEARNING.TESTSET_PATH = testSet_path
    testSet = data_obj.loadPartition(setPath=cfg.ACTIVE_LEARNING.TESTSET_PATH)
    testSet_loader = data_obj.getIndexesDataLoader(indexes=testSet, batch_size=cfg.TEST.BATCH_SIZE, data=test_data)
    # test_loader = data_obj.getTestLoader(data=test_data, test_batch_size=cfg.TRAIN.BATCH_SIZE, seed_id=cfg.RNG_SEED)
    ...
    for cur_episode in range(0, cfg.ACTIVE_LEARNING.MAX_ITER+1):
        ...
        best_val_acc, best_val_epoch, checkpoint_file = train_model(lSet_loader, testSet_loader, model, optimizer, cfg,  valSet_loader)
        print(model)
        ...
        # episode_sum_train = rankk(cur_episode, best_val_acc, 1.1)
        # total_sum_train += episode_sum_train
        # rankk_train_vall.append(total_sum_train)
        # print("episode train sum is: {}".format(episode_sum_train))
        # print("total current train sum is {}".format(total_sum_train))

        # Test best model checkpoint
        print("======== TESTING ========\n")
        logger.info("======== TESTING ========\n")
        # test_acc = test_model(testSet_loader, checkpoint_file, cfg, cur_episode, model)
        test_acc = test_model(valSet_loader, checkpoint_file, cfg, best_val_epoch, model)
        print("Test Accuracy: {}.\n".format(round(test_acc, 4)))
        logger.info("EPISODE {} Test Accuracy {}.\n".format(cur_episode, test_acc))
        # episode_sum = rankk(cur_episode,test_acc,1.1)
        # total_sum += episode_sum
        # rankk_test_vall.append(total_sum)
        # print("episode train sum is: {}".format(episode_sum))
        # print("total current train sum is {}".format(total_sum))
...
def train_model(train_loader, val_loader, model, optimizer, cfg, test_loader):
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            if temp_best_val_acc < val_set_acc:
                ...
                # # print("Model 2", model)
                # test_loader.dataset.no_aug = True
                # # test_err = test_epoch(test_loader, model, test_meter, cur_episode)
                # test_err = test_epoch(test_loader, model, test_meter, cur_epoch)
                # print("test_err line 431", test_err)
