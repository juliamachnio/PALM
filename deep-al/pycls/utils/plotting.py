...
# from pycls.utils.logging import load_json_stats, parse_json_stats
...
def prepare_plot_data(log_files, names, metric="top1_err"):
    ...
    for file, name in zip(log_files, names):
        d = {}
        log = logging.load_json_stats(file)  # Load log data

        # Extract data for train and test phases
        for phase in ["train", "test"]:
            x_key = "epoch"
            y_key = metric
            x = logging.parse_json_stats(log, phase, x_key)
            y = logging.parse_json_stats(log, phase, y_key)

            # Store data for plotting
            d[f"x_{phase}"], d[f"y_{phase}"] = x, y
            d[f"{phase}_label"] = f"[{min(y):5.2f}] {name}" if y else f"{name}"
