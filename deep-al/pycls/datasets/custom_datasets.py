...
class CIFAR100(torchvision.datasets.CIFAR100):
    def __init__(self, root, train, transform, test_transform, download=True, only_features=False, method='simclr'):
        super(CIFAR100, self).__init__(root, train, transform=transform, download=download)
        self.test_transform = test_transform
        self.no_aug = False
        self.only_features = only_features
        if only_features:
            self.features = ds_utils.load_features("CIFAR100", train=train, normalized=False, method=method)

            print("only_features 2", self.only_features)
    def __getitem__(self, index: int):
        """
        Args:
            index (int): Index
        ...
        # to return a PIL Image
        img = Image.fromarray(img)
        # print("only_features 3", self.only_features)
        if self.only_features:
            img = self.features[index]
        else:
            if self.no_aug:
