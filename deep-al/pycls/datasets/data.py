...
class Data:
    ...
    def getDataset(self, save_dir, isTrain=True, isDownload=False):
        ...
        if self.dataset == "MNIST":
            ...
            cifar100 = CIFAR100(save_dir, train=isTrain, transform=preprocess_steps,  test_transform=test_preprocess_steps, download=isDownload, only_features=only_features, method=self.setup)
            ...
            if isTrain:
                ...
                save_dir = os.path.join(save_dir, "ILSVRC/Data/CLS-LOC/train/")

                imagenet = ImageNet(save_dir, subset_file=subset_file, split='train', transform=preprocess_steps, test_transform=test_preprocess_steps,
    ...
    def makeTestSet(self, data, seed_id, save_dir):
        """
        Initialize the train and validation sets by splitting the train data according to split_ratios arguments.
        ...
        assert self.dataset in self.datasets_accepted, "Sorry the dataset {} is not supported. Currently we support {}".format(
            self.dataset, self.datasets_accepted)
        ...
        testSet = np.array(all_idx, dtype=np.ndarray)


        np.save(f'{save_dir}/testSet.npy', testSet)

        return f'{save_dir}/testSet.npy'
